...
class Backtest_Portfolio:
        ...
        # index=None,
        # compare_with = [],
        # df_marketcap = pd.DataFrame({'date':[]}),
        # save = False,
        # result_path="../results"
    
    def __init__(self, tickers_list, start_date, end_date, df_tics_daily, df_marketcap = None):
        self.tickers_list = tickers_list
        # self.indices_list = INDICES_LIST
        self.n_tickers = len(self.tickers_list)
        self.df_marketcap = df_marketcap
        self.compare_with_indices = True
        self.dropna = True
        self.fillna = False
        self.start_date = pd.to_datetime(start_date)
        self.end_date = pd.to_datetime(end_date)
        ...
        df_tics_daily = df_tics_daily[(df_tics_daily['date'] >= self.start_date) & (df_tics_daily['date'] <= self.end_date)]
        self.df_tics_daily = df_tics_daily.loc[df_tics_daily['tic'].isin(self.tickers_list)]
        # self.df_indices_daily = df_tics_daily.loc[df_tics_daily['tic'].isin(self.indices_list)]

    def equal_weights(self):
        weights = np.full(self.n_tickers, 1 / self.n_tickers)
        return weights

    def portfolio_return(self, weight_scheme = 'equal-weight'):
        # n_tickers = len(self.df_tics_daily.tic.unique())
        df_tics_close = pd.pivot_table(self.df_tics_daily, values='close',index='date',columns='tic')
        ...
        portfolio_returns = df_tics_returns.dot(portfolio_weights)
        portfolio_returns = pd.DataFrame(portfolio_returns,columns=['portfolio_returns'])

        # if self.compare_with_indices:
        #     df_index_close = pd.pivot_table(self.df_indices_daily, values='close',index='date',columns='tic')
        #     if dropna:
...
def scale_group(x, df, group_number):
    ...
    if group_number == 1:
        if x < 0:
            ...
            if min_val == max_val:  # Check for zero division
...
def process_industry_dataframe_v2(df, formation_date, industry):
    ...
    # print(f"{industry} NaN value count = {df_filtered.isna().sum().sum()}")
    ...
    df_filtered.to_excel('./examples/Quant_Rating/debug/' + industry + '_' + formation_date.strftime('%Y-%m-%d') + '.xlsx', index=False)

    df_filtered.fillna(df_filtered.median(numeric_only=True), inplace=True)               # questionable (we may drop the na row)
    df_score = df_filtered.copy()

    # def scale_group1(x, df):           
    #     if x < 0:
    #         return -1  # For non-positive values
    #     else:
    #         min_val = df['priceToBookRatio'][df['priceToBookRatio'] > 0].min()
    #         max_val = df['priceToBookRatio'][df['priceToBookRatio'] > 0].max()
    #         # Normalize the positive value to [0, 1]
    #         normalized_value = (x - min_val) / (max_val - min_val)
    #         # Apply -x + 1 to the normalized value
    #         return -normalized_value + 1

    # def scale_group2(x, df):
    #     # Get min and max values for positive values of 'priceToBookRatio'
    ...
    #         # Normalize the negative value to [-1, 0]
    #         normalized_value = ((x - min_val) / (max_val - min_val)) * (-1)  # Scale to [-1, 0]
    #         return normalized_value
    #     else:
    ...
    # Apply the function to the 'priceToBookRatio' column in the DataFrame
    # df_score['priceToBookRatio'] = df_score['priceToBookRatio'].apply(lambda x: func_neg_reverse(x, df_score))
...
def score_bookvalue(values, ):
    values = pd.Series([
        27.2231338, 191.5198347, 45.05565363, 137.8524263, 46.01828663, 18.2429288, 
        16.74119298, 14.71176984, 9.202666667, 14.42896523, -1.288643147, 14.16517532, 
        21.75984745, -62.52328125, 198.03675, 27.10116084, 17.10151119, 57.29997766, 
        16.98204421, 9.540115321, 40.56593677, -3.954546384, 38.02287648, 14.99229094, 
        -1074.380222, 16.53792841, 57.75740139, 28.15164011, 6.695994005, 12.9473684, 
        8.881245679, 12.38961469, 24.97027427
    ])
    # Get positive values
    positive_values = values[values > 0]
    median_positive = np.median(positive_values)

    upper_limit = np.percentile(positive_values, 95)
    ...
    for i, value in enumerate(values):
        ...
        elif value > 0:  # Positive values
            # Scale value between 0 and max_value
            scaled_value = value / upper_limit
            # Apply linear transformation
            transformed_values[i] = -scaled_value + 1
        else:  # Zero case (optional, treat as needed)
            transformed_values[i] = 0  # Assuming y = -x + 1 where x = 0
    ...
    # Save to Excel
    file_path = "./debug/robust_values_earning1.xlsx"
    df_transformed.to_excel(file_path, index=False)
...
def compute_value_growth_score_US(formation_date, value_coeff, growth_coeff):
    ...
    for industry, df in industry_dfs.items():
        ...
        if (len(latest_data['symbol'].unique()) >= 7):
            # df['date'] = pd.to_datetime(df['date'])
            ...
            df_score = process_industry_dataframe_v1(df)
...
def compute_value_growth_score_IN(formation_date, value_coeff, growth_coeff):
    ...
    for industry, df in industry_dfs.items():
        ...
        if len(df['symbol'].unique()) >= 5:
            ...
            df_score = process_industry_dataframe(df,industry)
            # sum_i = sum_i + len(df_score)
            # print(f"Scores are computed for industry {industry}")
            df_score['industry'] = industry
            df_score['sector']  = df['sector'].unique()[0]
...
def load_factors_coefficients_US_v2():
    rf_value_coeffs_df = pd.read_excel(INDUSTRY_REGRESSION_DIR + 'value/rf_value_coeffs_df.xlsx')#change her for quarter path
    rf_growth_coeffs_df = pd.read_excel(INDUSTRY_REGRESSION_DIR + 'growth/rf_growth_coeffs_df.xlsx')#change her for quarter path
    # Rename the Unnamed: 0 column to metrics
